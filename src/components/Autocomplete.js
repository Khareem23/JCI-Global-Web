...
import { SetFromAction, SetToAction } from "../redux/actions/authaction";
import "../css-style.css";

const Autocomplete = (props) => {
    const { setflyingto, setflyingfrom, type, placeholder, page } = props
        ...
        if (isShow && input) {
          if (filtered.length) {
              if(type === "from")
              {
                if(page !== "tourlist") {
                  return (
                    <ul className="autocomplete addposition2">
                  ...
                  return (
                    <ul className="autocomplete addposition3">
                      {filtered.map((suggestion, index) => {
                        let className;
                        if (index === active) {
                          className = "active";
                ...
                if(page !== "tourlist") {
                  ...
                  return (
                    <ul className="autocomplete addposition4">
                      {filtered.map((suggestion, index) => {
                        let className;
                        if (index === active) {
                          className = "active";
            ...
            if(type === "from")
            {
              return (
                <div className="no-autocomplete addposition2">
                  <em style={{marginLeft: '80px'}}>No data yet or not found</em>
                </div>
              );
            } else {
              return (
